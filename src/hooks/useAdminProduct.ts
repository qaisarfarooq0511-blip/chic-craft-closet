...
import type { Product, ProductImage, ProductInclude, ProductPiece } from "@/types/database";
...
async function fetchAdminProduct(id: string): Promise<AdminProductDetail | null> {
  const { data, error } = await supabase
    .from("products")
    .select("*, images:product_images(*), pieces:product_pieces(*), includes:product_includes(*)")
    .eq("id", id)
    .is("deleted_at", null)
    .maybeSingle();
